...
<script lang="ts">
  ...
  // week/month buckets so the chart stays readable.
  import { t, getLocale } from '$lib/i18n';
  import { line as d3Line, curveMonotoneX } from 'd3-shape';
  import { X } from '@lucide/svelte';
  import { buildAppUrl } from '$lib/utils/urlHelpers';
  import { formatDetectionCount } from './format';
  import {
    RANGE_PRESETS,
    type Bucket,
    type BucketPoint,
  ...
  const RANGE_LABELS = $derived({
    '7d': t('analytics.periods.lastWeek'),
    '30d': t('analytics.periods.lastMonth'),
    '90d': t('analytics.periods.last90Days'),
    '1y': t('analytics.periods.lastYear'),
    '2y': t('analytics.speciesTools.historyTwoYears'),
    all: t('analytics.periods.allTime'),
  });
  ...
  const subtitle = $derived.by(() => {
    if (showError) return t('common.ui.error');
    const v = displayed;
    if (!v) return t('common.ui.loading');
    let text = t('analytics.speciesTools.historySummary', {
      count: formatDetectionCount(v.total),
      range: RANGE_LABELS[v.range],
    });
    if (v.range === 'all' && fullData?.firstMs != null) {
      const date = new Date(fullData.firstMs).toLocaleDateString(getLocale(), { timeZone: 'UTC' });
      text += ` (${t('analytics.speciesTools.historySince', { date })})`;
    }
    return text;
  });
  ...
  function formatAvg(avg: number): string {
    if (avg === 0) return '0';
    if (avg < 0.1) return `<${(0.1).toLocaleString(getLocale())}`;
    if (avg < 10)
      return avg.toLocaleString(getLocale(), {
...
<div
  ...
  tabindex="-1"
  aria-modal="true"
  aria-label={t('analytics.speciesTools.historyTitle', { name: displayName })}
...
>
  <div class="hist-panel">
    ...
    <div class="hist-header">
      ...
      <button class="hist-close" aria-label={t('common.buttons.close')} onclick={onClose}>
        <X class="size-4" />
      </button>
    </div>
    ...
    {#if displayed && !displayed.empty}
      <div class="hist-stats" class:stale={refreshing}>
        <div class="hist-stat">
          <span class="hist-stat-label">{t('dataDisplay.stats.total')}</span>
          <span class="hist-stat-value">{formatDetectionCount(displayed.total)}</span>
        </div>
        <div class="hist-stat">
          <span class="hist-stat-label">{t('analytics.speciesTools.historyPeak')}</span>
        ...
        <div class="hist-stat">
          <span class="hist-stat-label">{t('analytics.speciesTools.historyAverage')}</span>
    ...
    <div class="hist-body">
      {#if showSkeleton}
        <div class="hist-skeleton" role="status" aria-label={t('common.ui.loading')}>
      ...
      {:else if showError}
        <div class="hist-state" role="alert">
          <span>{t('common.ui.error')}</span>
          <button class="hist-retry" onclick={() => (retryToken += 1)}>{t('common.retry')}</button>
        </div>
      {:else if displayed?.empty}
        <div class="hist-state">
          <span>{t('analytics.speciesTools.historyEmpty')}</span>
          {#if displayed.range !== 'all'}
            <span class="hist-state-hint">{t('analytics.speciesTools.historyLonger')}</span>
          {/if}
        </div>
      {:else if displayed}
        <div
        ...
        >
          {#if geom}
            ...
            <svg
              ...
              role="img"
              aria-label={t('analytics.speciesTools.historyChart', {
                name: displayName,
                count: displayed.total,
                range: RANGE_LABELS[displayed.range],
                peak: displayed.max,
              })}
          ...
          {/if}
          {#if refreshing}
            <div class="hist-refresh-pill" role="status">{t('common.ui.loading')}</div>
          {/if}
        </div>
        {#if displayed.showAvg}
          <div class="hist-legend">
            <span class="hist-legend-item"
              ><span class="hist-swatch-bar"></span>{t('analytics.speciesTools.historyDaily')}</span
            >
            <span class="hist-legend-item"
    ...
    <div class="hist-ranges" role="group" aria-label={t('analytics.filters.timePeriod')}>
