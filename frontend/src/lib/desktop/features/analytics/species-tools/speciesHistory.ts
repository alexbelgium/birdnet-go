...
export const RANGE_PRESETS: { key: RangeKey }[] = [
  { key: '7d' },
  { key: '30d' },
  { key: '90d' },
  { key: '1y' },
...
function monthShort(utcMs: number): string {
  return new Date(utcMs).toLocaleDateString(getLocale(), { month: 'short', timeZone: 'UTC' });
}

/** "Jun 12" style label for a single day. */
export function shortDayLabel(utcMs: number): string {
  return new Date(utcMs).toLocaleDateString(getLocale(), {
    month: 'short',
    day: 'numeric',
    timeZone: 'UTC',
  });
...
export function bucketLabel(bp: BucketPoint, bucket: Bucket): string {
  ...
  if (bucket === 'week') {
    return formatter.formatRange(new Date(bp.startMs), new Date(bp.endMs));
  }
  return formatter.format(new Date(bp.startMs));
}
