...
func TestResolveStreamSampleRates(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:       "probe ok, bat below capture floor raises output to floor",
			probeRate:  48000,
			isBat:      true,
			wantSource: 48000,
			wantOutput: batCaptureRate,
		},
		{
		...
		{
			name:       "probe failed, fallback below floor, bat retains source and captures at floor",
			probeRate:  0,
			fallback:   32000,
			isBat:      true,
			wantSource: 32000,
		...
		{
			name:       "probe failed, no fallback, bat escalates and captures at floor",
			probeRate:  0,
			fallback:   0,
			isBat:      true,
			wantSource: 0,
