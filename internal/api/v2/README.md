...
| POST   | `/detections/confirm`         | `ConfirmSpecies`        | ✅   | Toggle species in confirmed (verified) list |
| GET    | `/detections/confirmed`       | `GetConfirmedSpecies`   | ✅   | Get list of manually confirmed species      |
...
| GET    | `/range/species/scores` | `GetRangeFilterSpeciesScores` | ❌   | All species with raw geomodel scores (no threshold cutoff); `?names=false` skips localized common-name resolution for a faster scientific-name-only response |
