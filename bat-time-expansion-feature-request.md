...
**Improvement:** because BirdNET-Go's *native* expansion is `sourceRate/48000`, offering an **Auto/Native** preset (computed from the clip's probed sample rate) as the default is more honest to the pipeline than a flat 5×, and it lines the audio up 1:1 with the displayed spectrogram. The fixed 5/10/16/20× presets remain available.

## Backend implementation

Reuse existing infrastructure rather than a standalone service.

1. **Endpoint** — extend the existing audio-serving surface (`internal/api/v2/media.go`) instead of inventing a parallel one. Either:
...
2. **Probe** the source rate via the existing ffprobe path (`internal/audiocore/ffmpeg/validate.go`); reject/hide if `sampleRate ≤ 48000`.

3. **Generate** the derived 48 kHz file through the existing FFmpeg filter-chain mechanism (`internal/audiocore/ffmpeg/export.go` / `clip.go`), adding the `asetrate,aresample` filter. Never write to the original path.

4. **Cache** on disk keyed by `(noteID, sourceRate, factor)`, mirroring the spectrogram cache: immutable `Cache-Control` headers and a `sync.Map`-based in-flight dedup queue so concurrent requests don't regenerate the same file. Serve `audio/wav` with `Content-Disposition: inline`.
...
- **Visibility rule:** show the control only when the detection is high-sample-rate (probe result > 48 kHz, or the detection's model is the `bat` model). Otherwise hide it (optionally with an info tooltip).
...
**In scope (v1):** Original + Native/Auto + 5× + 10× + 16× + 20× presets; server-side generation; on-disk caching; clear "derived copy" labelling; 192/256/384 kHz source support; visibility gating.
...
- For a high-sample-rate bat detection, the user can play a 48 kHz audible time-expanded copy in the browser.
...
- The control is hidden (or shows an informative notice) when the source is ≤ 48 kHz.
